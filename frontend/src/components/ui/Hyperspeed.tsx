...
export const DEFAULT_EFFECT_OPTIONS: HyperspeedOptions = {
  onSpeedUp: () => {},
  onSlowDown: () => {},
...
const Hyperspeed: React.FC<HyperspeedProps> = ({ effectOptions = DEFAULT_EFFECT_OPTIONS, lightMode = false }) => {
  ...
  useEffect(() => {
    ...
    class CarLights {
      ...
      update(time: number) {
        if (this.mesh && this.mesh.material && this.mesh.material.uniforms && this.mesh.material.uniforms.uTime) {
  ...
  return <div id="lights" ref={hyperspeed} style={{ width: '100%', height: '100%', position: 'absolute', top: 0, left: 0 }}></div>;
};
