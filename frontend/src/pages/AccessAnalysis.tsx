  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight">Access Analysis</h1>
          <p className="text-xs sm:text-sm text-slate-400">Detailed breakdown of excessive and unused permissions</p>
        </div>
            ...
            <CardHeader className="p-4 sm:p-6 pb-2">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div className="flex flex-wrap items-center gap-2">
                  <ShieldAlert className={`h-4 w-4 sm:h-5 sm:w-5 shrink-0 ${finding.risk_level === 'CRITICAL' ? 'text-critical' : 'text-danger'}`} />
                  <CardTitle className="text-base sm:text-lg text-slate-200">{finding.role_name}</CardTitle>
                  <code className="px-2 py-0.5 bg-slate-950 text-slate-300 rounded text-xs font-mono border border-slate-800 break-all">{finding.action}</code>
                </div>
